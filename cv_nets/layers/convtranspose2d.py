...
class ConvTranspose2d(nn.ConvTranspose2d):
    def __init__(
        ...
        **kwargs: Any,
        ...
        _output_padding = get_param(opts, output_padding, "output_padding", 0)
    ...
    def add_arguments(cls, parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        ...
        group.add_argument(
            ...
            default=1, 
            metavar="N",
            help="Bước nhảy phóng to (default: 1)"
        )
        group.add_argument(
            "--padding", 
            type=int, 
            default=0, 
            metavar="N",
            help="Đệm cắt bớt viền đầu ra (default: 0)"
        )
        group.add_argument(
            "--output-padding", 
            type=int, 
            default=0, 
            metavar="N",
            help="Đệm bổ sung cho kích thước đầu ra (default: 0)"
        )
        group.add_argument(
            "--dilation", 
            type=int, 
        ...
        group.add_argument(
            ...
            default=1, 
            metavar="N",
            help="Số lượng nhóm (default: 1)"
        )
        group.add_argument(
            "--padding-mode", 
            type=str, 
            default="zeros", 
            choices=["zeros"], # PyTorch hiện tại chủ yếu hỗ trợ 'zeros' cho Transpose
            help="Chế độ đệm (default: zeros)"
        )
        group.add_argument(
            "--bias", 
            action="store_true", 
            help="Sử dụng bias cho lớp ConvTranspose2d"
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Kiểm tra thông số của lớp ConvTranspose2d",
        formatter_class=formatter
    )
    ConvTranspose2d.add_arguments(parser)
