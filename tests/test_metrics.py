...
class TestCalculateIoU:
    def test_perfect_match(self):
        ...
        Ghi chú: calculate_iou nhận logits (chưa sigmoid), không phải xác suất.
        """
        pred = torch.full((2, 1, 32, 32), 10.0)  # logit lớn → sigmoid ≈ 1
    ...
    def test_no_match(self):
        ...
        pred = torch.full((2, 1, 32, 32), -10.0)  # logit âm → sigmoid ≈ 0
        mask = torch.ones(2, 1, 32, 32)
        iou = calculate_iou(pred, mask)
    ...
    def test_half_overlap(self):
        """IoU cho overlap 50%."""
        pred = torch.full((1, 1, 32, 32), -10.0)
    ...
    def test_different_threshold(self):
        ...
        Dùng logit=0 (sigmoid=0.5) và threshold lân cận để kiểm tra.
        """
        pred = torch.zeros(1, 1, 32, 32)  # logit=0 → sigmoid(0)=0.5
        mask = torch.ones(1, 1, 32, 32)
        # Với threshold=0.6: sigmoid(0)=0.5 < 0.6 → pred=0 → IoU=0
        iou_strict = calculate_iou(pred, mask, threshold=0.6)
        # Với threshold=0.4: sigmoid(0)=0.5 > 0.4 → pred=1 → IoU=1
        iou_lenient = calculate_iou(pred, mask, threshold=0.4)
...
class TestCalculateDice:
    def test_dice_equals_iou_when_perfect(self):
        ...
        Ghi chú: calculate_* nhận logits (chưa sigmoid).
        """
        pred = torch.full((2, 1, 32, 32), 10.0)  # logit → sigmoid ≈ 1
        mask = torch.ones(2, 1, 32, 32)
    ...
    def test_dice_higher_than_iou(self):
        ...
        pred = torch.randn(4, 1, 32, 32)  # raw logits
